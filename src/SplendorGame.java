...
public class SplendorGame extends JPanel implements MouseListener {
	...
	public SplendorGame(int size, SplendorFrame frame) {
	...
	public void makePatrons() {
		ArrayList<Patron> patronDeck = new ArrayList<Patron>(); // temporary patron deck that will contain all patrons
																// from the csv file
	...
	public void paint(Graphics g) {
		...
		if (errorPanel == false) {
			...
			for (int i = 0; i < tokens.length; i++) {
				...
				g.drawImage(Card.numbers[tokens[i]],
						(tokensX + tokenPadding * i) + ((tokenButtons[i].getWidth() - textSize) / 2),
						tokensY + ((tokenButtons[i].getHeight() - textSize) / 2), textSize, textSize, null);
			}
			...
			if (size != 2 && patrons[3] != null) {
				patrons[3].setPosition(patronsX + patronsPadding,
						patronsY + patronsRectHeight / 2 - patronsPadding / 2);
				patrons[3].paint(g);
			}

			if (size == 4 && patrons[4] != null) {
				patrons[4].setPosition(patronsX + patronsPadding,
						patronsY + patronsRectHeight / 2 + patronsPadding / 2);
				patrons[4].paint(g);
			}
			...
			for (int i = 0; i < currentTokens.length; i++) {
				if (currentTokens[i] >= 0) {
					...
					g.drawImage(Card.numbers[currentTokens[i]], xPos + (currentTokenSize - currentTokenTextSize) / 2,
							yPos + (currentTokenSize - currentTokenTextSize) / 2, currentTokenTextSize,
							currentTokenTextSize, null);
				}
			}
			...
			g.drawImage(Card.numbers[turn + 1], currentPlayerX + currentPlayerWidth + currentPlayerSpace,
					currentPlayerY, currentPlayerHeight, currentPlayerHeight, null);
			...
			for (int i = 0; i < otherTokens.length; i++) {
				if (otherTokens[i] >= 0) {
					...
					g.drawImage(Card.numbers[otherTokens[i]], xPos + (otherTokensSize - otherTokensTextSize) / 2,
							yPos + (otherTokensSize - otherTokensTextSize) / 2, otherTokensTextSize,
							otherTokensTextSize, null);
				}
			}
			...
			g.drawImage(Card.numbers[otherPlayer + 1], otherPlayerX + otherPlayerWidth + otherPlayerSpace, otherPlayerY,
					otherPlayerHeight, otherPlayerHeight, null);
			...
			if (showOtherTab) {
				...
				g.drawImage(otherTabs[otherPlayerViewing], otherTabX, otherTabHiddenY, otherTabWidth, otherTabHeight,
						null);
			...
			case CARDS:
				...
				for (int i = 0; i < currentCards.length; i++) {
					for (int j = 0; j < currentCards[i].size(); j++) {
						...
						card.setPosition(currentCardsX + currentCardsPaddingX * j,
								currentCardsY + currentCardsPaddingY * i);
						card.paint(g);
					}
				}
			...
			if (showOtherTab) {
				...
				case CARDS:
					...
					for (int i = 0; i < currentCards.length; i++) {
						for (int j = 0; j < currentCards[i].size(); j++) {
							Card card = currentCards[i].get(j);
							card.scale(currentCardsScale, currentCardsScale);
							card.setPosition(currentCardsX + currentCardsPaddingX * i + currentCardsPaddingExtra * j,
									currentCardsY);
							card.paint(g);
						}
					}
				...
				case RESERVED:
					...
					for (int i = 0; i < currentReserved.size(); i++) {
						Card card = currentReserved.get(i);
						g.drawImage(cardBacks[card.getLevel() - 1], currentReservedX + currentReservedPadding * i,
								currentReservedY, (int) (Card.WIDTH * currentReservedScale),
								(int) (Card.HEIGHT * currentReservedScale), null);
					}
					break;
				}
			}
			...
			g.drawImage(errorScreen, 250, 72, errorScreen.getWidth() / 2, errorScreen.getHeight() / 2, null); // paint
																												// error
																												// window
			okButton.paint(g);
	...
	public void endGame() { // player has mroe than 15 points will be called every turn after u get 15 to
							// check if u need to like end the whole game
		System.out.println("game is ending");
		if (endingTurns == players.length - 1) {
			frame.addEnd(players);
			System.out.println("panel switched to end game");
		}
	...
	public void buyCard(Card c, int index) {
		int[] x = players[turn].buyCard(c);
		if (x != null) { // if player does buy card
			int[] price = c.getPrice();
			int[] gems = players[turn].getGems();

			gems[c.getGem()]--; // remove one gem from the color that the card was boight bc it was just added
								// ykyk
			canBuyCard = false;
	...
	public void buyPatron() { // wut u ca;l after an action to check if u can buy patron and then go to next
								// turn

		if (players[turn].getTotalTokenCount() > 10) { // check if t hey acceed limit
			replaceTokens = true;
			errorScreen(MAXTOKENS);
			return;
	...
	public void buyReservedCards(Card c, int index) {
		...
		if (x != null) { // if player does buy card
			...
			gems[c.getGem()]--; // remove one gem from the color that the card was boight bc it was just added
								// ykyk
			canBuyCard = false;
			for (int i = 0; i < 6; i++) { // add back tokens to game
				tokens[i] += x[i];
			}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (errorPanel == false) { // if error screen is not showing curretnly
			...
			for (int i = 0; i < tokenButtons.length - 1; i++) { // Clicking Normal tokens
				if (tokenButtons[i].isInside(x, y)) {
					...
					if (tokenClickCounter == 0) {
						...
						boolean hasOtherTokens = false;
						int clickedToken = 0;
						for (int j = 0; j < tokens.length - 1; j++) {
							if (tokens[j] > 0 && tokenClickCount[j] < 1) {
								hasOtherTokens = true;
							}
							if (tokenClickCount[j] > 0) {
								clickedToken = j;
							}
						}

						if (!hasOtherTokens && tokens[clickedToken] < 3) {
							buyPatron();
							break;
						}
					} else if (tokenClickCounter == 1) {

						if (tokenClickCount[i] >= 1 && tokens[i] >= 3) {
							tokenClickCount[i]++;
							tokenClickCounter++;
							tokens[i]--;
							players[turn].addToken(i);
							buyPatron();
						} else if (tokenClickCount[i] >= 1 && tokens[i] < 3) {
							errorScreen();
						} else if (tokenClickCount[i] == 0) {
							tokenClickCount[i]++;
							tokenClickCounter++;
							tokens[i]--;
							players[turn].addToken(i);
			...
			if (tokenButtons[5].isInside(x, y)) { // Golden tokens, WIP
				if (alreadyClickedYellow || players[turn].getReservedCards().size() >= 3)
					errorScreen();
				else {
